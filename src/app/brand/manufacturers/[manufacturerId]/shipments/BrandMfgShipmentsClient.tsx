...
import { X, Plus, Check, Eye, Upload, FileText, Trash2 } from "lucide-react";
import { createShipmentAction } from "@/app/brand/shipments/actions";
...
function CreateShipmentModal({
  ...
  async function handleSubmit() {
    ...
    const result = await createShipmentAction({
      manufacturerId,
      shippingMethod,
      statusReportItemIds: [...selectedIds],
...
function ViewDetailsModal({ item, onClose }: { item: BrandSRRow; onClose: () => void }) {
...
export function BrandMfgShipmentsClient({
  manufacturerId,
  acceptedItems,
  readyForPickupItems,
  shipmentRequests,
  ...
  manufacturerId: string;
  acceptedItems: AcceptedRow[];
  readyForPickupItems: BrandSRRow[];
  shipmentRequests: MfgShipmentRequest[];
}) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<(typeof SHIPMENT_TABS)[number]>("Accepted");
  const [showShipmentModal, setShowShipmentModal] = useState(false);
  const [viewDetailsItem, setViewDetailsItem] = useState<BrandSRRow | null>(null);

  const thClass = "text-left px-5 py-3 text-[11px] uppercase tracking-wider text-text-muted font-medium";

  const readyItems = readyForPickupItems.filter((i) => !i.hasShipmentRequest);
  ...
  function renderItemsTable(items: BrandSRRow[], emptyMessage: string) {
    if (items.length === 0) {
      return (
  ...
  function renderAcceptedTable(rows: AcceptedRow[], emptyMessage: string) {
    if (rows.length === 0) {
      return (
        <div className="bg-bg-card border border-border rounded-xl p-8 text-center">
          <p className="text-text-muted text-sm">{emptyMessage}</p>
  ...
  function renderShipmentTable(requests: MfgShipmentRequest[], emptyMessage: string) {
  ...
  return (
    <>
      ...
      {activeTab === "Ready for Pickup" && renderItemsTable(readyItems, "No items ready for pickup.")}
